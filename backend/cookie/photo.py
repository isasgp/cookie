...
def upload_photo(request):
    ...
    if 'temp_image' in request.FILES:
        ...
        text = model_views.process_uploaded_image(file_path) # - addition ! -
        
        # index.html ???????? ????????
        return Response({'message':text}, status=status.HTTP_201_CREATED) # 원래 문구 : ????? ???로드 ?????
    # else:
        # return JsonResponse({'message': text}, status=201) # 원래 문구 : ????????? ????????????.
